...
import { APP_NAME, DOMAIN, MANGA_NAME, NEXT_PREVIOUS_PREFIX, IMAGE_PREFIX, CHAPTER_PREFIX, AUTHOR_PAGE, LOGO_URL, chaptersData, IMAGES_DOMAIN } from '@/config';
...
export default function Chapter({ chapterNumber, imageUrls, totalChapters, params, errorcode }) {
    ...
    const DESCRIPTION = `Enjoy reading ${MANGA_NAME} chapter ${chapterNumber} online at ${DOMAIN}. The manga follows Monkey D. Luffy and his pirate crew as they search for the legendary treasure, One Piece, to become the Pirate King. The series is known for its epic adventures, rich world-building, and diverse characters.`
    ...
    const schema =
    {
        ...
        "image": `https://images.onepiecemanga.in.net/one-piece/chapter-${chapterNumber}/1.webp`,
        "author": {
            "@type": "Person",
    ...
    const head = () => (
        <Head>
            ...
            <meta property="og:image" content={`https://images.onepiecemanga.in.net/one-piece/chapter-${chapterNumber}/1.webp`} />
            <meta property="og:image:secure_url" content={`https://images.onepiecemanga.in.net/one-piece/chapter-${chapterNumber}/1.webp`} />
...
export async function getStaticProps({ params }) {
    ...
    const chapterNumber = chapterParam.split(`one-piece-chapter-`)[1];
...
const getImageUrls = (chapterNumber, numImages) => {
    const imageUrls = [];
    // const numericPart = parseInt(chapterNumber, 10);
    let chapterImagesFolder;

    for (let i = 1; i <= numImages; i++) {
        chapterImagesFolder = `${IMAGES_DOMAIN}-${chapterNumber}`;
